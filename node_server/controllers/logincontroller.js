...
module.exports.apple_login=function(req,res){

	var apple_id = req.body.apple_id;

	connection.query('SELECT * FROM users WHERE apple_id = ?',[apple_id], function (error, results, fields) {
		if (error) 
		{
			res.json({status:false,status_code:201,message:'there are some error with query'})
		} 
			...
			else 
			{
				...
				connection.query('SELECT * FROM users WHERE email = ?',[email], function (error, results, fields) {
					...
					else 
					{
						...
						{
							var sql = "INSERT INTO users (first_name, name, email, role, apple_id, status) VALUES ('"+name+"','"+name+"','"+email+"','"+role+"','"+apple_id+"','"+status+"')";
							connection.query(sql, function (err, result) 
							{
						    	if (err) throw err;
